...
module.exports = {
    ...
    let sql = `SELECT * from songcopy ORDER BY sid desc limit 9;`;
    ...
    conn.end();
        ...
        getJoinLists: function(callback){
            let conn = this.getConnection();
            let sql = `
                SELECT songcopy.sid, songcopy.title, gg.NAME, 
                songcopy.lyrics from songcopy
                left JOIN girl_group AS gg
                ON songcopy.sid=gg.hit_song_id
                ORDER BY songcopy.sid DESC
                LIMIT 9;
                    `;
    conn.query(sql, (error, rows, fields) =>{
        if(error)
